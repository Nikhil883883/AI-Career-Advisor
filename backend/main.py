...
class ChatRequest(BaseModel):
    message: str
    model: Optional[str] = "llama3-8b-8192"  # Default model
class ResumeVettingRequest(BaseModel):
    resume_text: Optional[str] = None
    resume_pdf: Optional[bytes] = None
    job_description: Optional[str] = None
    model: Optional[str] = "llama3-70b-8192"
    analysis_type: Optional[List[str]] = ["ats", "skills", "structure"]  # New: Specify analysis types
...
async def chat_with_bot(payload: ChatRequest):
    """
    Chat with AI assistant about career-related questions.
    """
    try:
...
async def recommend_career(
    ...
    """
    Get personalized career recommendations based on either:
    - Uploaded resume PDF, or
    - Detailed form fields, or
    - Combination of both
    """
    try:
        ...
        # 2. Build profile from form fields
        form_fields = {
            "name": name,
            "ageGroup": ageGroup,
...
async def groq_job_search(query: str = "software engineer", location: str = "remote"):
    """
    Get AI-generated job market insights using only Groq
    Example: /jobs/?query=python&location=india
    """
    try:
        prompt = f"""
...
async def vet_resume(
    ...
    nocache: str = Form(None)  # Cache buster from frontend
):
    try:
        # 1. Process input
        ...
        # 2. Create analysis prompt with strict JSON formatting
        ...
        # 3. Get Groq response with retry logic
        max_retries = 2
        for attempt in range(max_retries):
            try:
                groq_response = call_groq_api(prompt, "llama3-70b-8192")
                
                # 4. Strict JSON parsing with validation
        ...
        return JSONResponse(
            ...
            content={
                ...
                "data": {
                    ...
                    "analysis_id": nocache  # Echo back for debugging
                }
            }
        )
